...
export async function POST(request) {
    ...
    try {
        ...
        const res = await sql.query(`update targetdate set shareid='${shareid}' WHERE id = ${id} returning shareid`);
        const shareID = res[0].shareid;
        const date = new Date().toLocaleString("en-US", { timeZone: "Asia/Kathmandu" });
        await sql.query(`INSERT INTO notifications (title, created_at, category, label) VALUES ('Target share id created with id ${shareID}', '${date}','shareidcreated','Share ID Created')`);
